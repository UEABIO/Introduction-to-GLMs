...
When using our residuals to calculate standard error, confidence intervals and statistical significance these are assumed to be drawn from a normal distribution with mean zero and a constant variance. 
...
- The Generalized Linear Model (GLM) is an extension of the ordinary linear regression model that accommodates a broader range of response variable types and error distributions.
...
- Rate (β): Influences the scale or spread of the distribution. Smaller values of β result in distributions that are more spread out, while larger values of β lead to distributions that are more concentrated around the mean.
...
- Which estimates have the are most probable for producing the observed data
...
- It measures the deviance of the fitted generalized linear model with respect to a perfect/**saturated** model for the sample.
...
- AIC is calculated as follows: $\text{AIC} = 2k - 2\ln(\hat{L})$, where:
log-likelihood is the maximized value of the likelihood function for the fitted model.
...
- The likelihood ratio test is typically calculated as twice the difference in deviance between the two models.
...
* Check model fits and report findings
...
Terms are currently not on the same scale as the response variable:

- Exponentiated coefficients allow for direct comparison of the effects of predictors on the response variable.

- Be clear and careful about which you are using
...
After an investigation into the incident, it was discovered that the failure was caused by critical failurs in all six of the O-rings in the solid rocket booster. Additionally, it was revealed that such an incident was foreseeable.
